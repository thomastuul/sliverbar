...
int main(int argc, char **argv) {
  ...
  CHECK(powerActionRetryDelay(0) == 0);
  CHECK(powerActionRetryDelay(1) == 1);
  CHECK(powerActionRetryDelay(2) == 2);
  CHECK(powerActionRetryDelay(3) == 4);
  CHECK(powerActionRetryDelay(4) == 8);
  CHECK(powerActionRetryDelay(5) == 16);
  CHECK(powerActionRetryDelay(6) == 32);
  CHECK(powerActionRetryDelay(7) == 0);
  PanelState staticState = {0};
  snprintf(cfg.powerMenuMode, sizeof(cfg.powerMenuMode), "internal");
  cfg.internalPowerAvailable = false;
  moduleStatic(&cfg, &staticState);
  CHECK(staticState.power[0] == '\0');
