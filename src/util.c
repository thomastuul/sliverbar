...
bool command_exists(const char *name) {
    ...
    char *save = NULL;
    for (char *d = strtok_r(copybuf, ":", &save); d; d = strtok_r(NULL, ":", &save)) {
...
int run_capture(char *const argv[], char *out, size_t size, int timeout_ms) {
    ...
    for (;;) {
        ssize_t n = read(pipefd[0], out + used, size && used < size - 1 ? size - used - 1 : 0);
        if (n > 0)
            used += (size_t)n;
        else
            break;
...
void shell_quote_action(const char *in, char *out, size_t size) {
    size_t n = 0;
    for (; *in && n + 2 < size; in++) {
        char c = *in;
        if (c == ':' || c == '|' || c == '%' || c == '\n' || c == '\r')
